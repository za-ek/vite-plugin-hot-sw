...
export default function hotServiceWorkerPlugin(instanceOptions: HotServiceWorkerOptions = {}): PluginOption {
    ...
    const doHotMake = (server: ViteDevServer) => {
        doMake();
        hotCopy(server.config.root).then(() => {
            server.ws.send({ type: "full-reload", path: "*" });
        })
    };
    const injectAssets = (assets: string[], serviceWorkerPath: string) => {
        try {
            let content = fs
    ...
    const prepareConfig = (configFile: string, serviceWorkerFileName: string) => {
        const confContent = fs.readFileSync(path.resolve(__dirname, 'vite-sw.config.js')).toString()
        fs.writeFileSync(
            configFile,
            confContent.replace('#SERVICE_WORKER_FILE_NAME#', path.join('.', serviceWorkerFileName).replace(/[\\$'"]/g, "\\$&"))
        )
    }
